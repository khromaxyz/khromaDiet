...
export const GoalSlide = ({ activated, results, formData }: GoalSlideProps) => {
  ...
  const goalPct =
    results.tdeeFinal > 0 ? Math.max(8, Math.min(100, (results.goalCalories / results.tdeeFinal) * 100)) : 100;
  const deltaPct = Math.max(4, Math.min(100, Math.abs(results.goalDeltaPct)));

  const rateAnimated = useCountUp(Math.round(Math.abs(results.weeklyRateKg) * 100), activated, 900) / 100;
      ...
      >
        ...
        <motion.div variants={dashboardPanelVariants}>
          ...
          >
            <div className="space-y-4">
              <div>
                ...
                <div className="mt-2 max-w-[38rem] text-[15px] leading-[1.7] text-[var(--text-secondary)]">
                  A manutencao fica no topo da trilha e a meta mostra exatamente quanto do TDEE continua no prato.
                  O restante representa o ajuste diario do protocolo.
                </div>
              </div>

              <div className="rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--bg-deep)] p-[var(--space-5)]">
                <div className="mb-4 flex items-center justify-between gap-4">
                  <div className="text-sm text-[var(--text-secondary)]">Meta como percentual da manutencao</div>
                ...
                <div className="relative h-4 overflow-hidden rounded-full border border-[var(--border-default)] bg-[var(--bg-active)]">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ width: activated ? `${goalPct}%` : '0%' }}
                  ...
                  <motion.div
                    ...
                    animate={{
                      left: activated ? `${goalPct}%` : '0%',
                      width: activated ? `${Math.max(maintenancePct - goalPct, 0)}%` : '0%',
                    }}
                    transition={dashboardBarTransition}
                  />
                </div>
            ...
            <motion.div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1" variants={dashboardStaggerGroupVariants}>
              <motion.div
              ...
              >
                ...
                <div className="font-mono text-[20px] font-semibold tracking-[-0.8px] text-[var(--text-primary)]">
                  {formatKcal(results.tdeeFinal)} - {formatKcal(Math.abs(results.dailyDelta))} = {formatKcal(results.goalCalories)}
                </div>
              </motion.div>
