...
const formatSignedKcal = (value: number) => `${value >= 0 ? '-' : '+'}${formatKcal(Math.abs(value))}`;

export const FinalSlide = ({ activated, isActive, results, formData, profileMeta }: FinalSlideProps) => {
  ...
  const estimatedWeeks = results.projection?.milestones.at(-1)?.week ?? formData.targetWeeks ?? null;
  ...
  const generatedAtLabel = formatShortDate(profileMeta?.createdAt ?? new Date().toISOString()) ?? 'Plano atual';
    ...
    () => [
      {
        id: 'tdee',
        label: 'TDEE',
        value: formatKcal(results.tdeeFinal),
        unit: 'kcal',
      ...
      },
      {
        id: 'goal',
        label: 'Meta',
        value: formatKcal(results.goalCalories),
        unit: 'kcal',
      ...
      },
      {
        ...
        value: formatSignedKcal(results.dailyDelta),
      ...
      },
      {
        id: 'protein',
        label: 'Proteina',
        value: formatKcal(results.macros.proteinG),
        unit: 'g',
      ...
      },
      {
        id: 'carbs',
        label: 'Carboidrato',
        value: formatKcal(results.macros.carbsG),
        unit: 'g',
      ...
      },
      {
        id: 'fat',
        label: 'Gordura',
        value: formatKcal(results.macros.fatG),
        unit: 'g',
      ...
      },
      {
        id: 'pace',
        label: 'Ritmo',
        value: formatPct(Math.abs(results.weeklyRateKg), 2),
        unit: 'kg/sem',
    ...
    const sharedProfile =
      profileMeta?.name || typeof profileMeta?.avatarId === 'number'
        ? {
            ...
            ...(typeof profileMeta?.avatarId === 'number' ? { avatarId: profileMeta.avatarId } : {}),
          }
        : undefined;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]" variants={dashboardPanelVariants}>
          <DataCard
          ...
          >
            <div className="space-y-4">
              ...
              <p className="max-w-[42rem] text-[15px] leading-[1.7] text-[var(--text-secondary)]">
                O proximo passo do produto vai usar o protocolo ja validado para montar a dieta completa com distribuicao pratica e exportacao pronta.
              </p>
            ...
            <div className="flex h-full flex-col justify-between gap-5">
              <div className="space-y-3">
                ...
                <div className="text-[15px] leading-[1.7] text-[var(--text-secondary)]">
                  Exporte o dashboard inteiro para PDF ou gere um link para compartilhar o plano atual sem depender do legado.
                </div>
              </div>
              ...
              {shareStatus ? (
                <Badge
                  data-testid="final-share-status"
                  variant={shareTone}
                  className="w-fit"
                >
                  {shareStatus}
