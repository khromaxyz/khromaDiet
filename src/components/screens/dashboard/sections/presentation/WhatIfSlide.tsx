...
import '../../../../../styles/dashboard-simulator.css';
import { LEGACY_SIMULATOR_HTML, LEGACY_SIMULATOR_SCRIPT } from './simulatorLegacy.generated';
...
export const WhatIfSlide = ({ isActive }: WhatIfSlideProps) => {
  ...
  useEffect(() => {
    ...
    content.innerHTML = LEGACY_SIMULATOR_HTML;
    ...
    const trackSetTimeout: typeof window.setTimeout = ((handler: TimerHandler, timeout?: number, ...args: any[]) => {
      const safeHandler =
        typeof handler === 'function'
          ? (runSafely('Simulator legacy runtime callback error:', handler as (...handlerArgs: any[]) => unknown) as TimerHandler)
          : handler;
    ...
    const trackRequestAnimationFrame: typeof window.requestAnimationFrame = ((callback: FrameRequestCallback) => {
      const safeCallback = runSafely('Simulator legacy runtime callback error:', callback);
    ...
    const trackWindowAddEventListener = (
      ...
      const wrapped = toSafeEventListener('Simulator legacy window listener error:', listener);
    ...
    const trackDocumentAddEventListener = (
      ...
      if (type === 'DOMContentLoaded') {
        const wrapped = toSafeEventListener('Simulator legacy DOMContentLoaded handler error:', listener);
        wrapped(new Event('DOMContentLoaded'));
        return;
      }

      const wrapped = toSafeEventListener('Simulator legacy document listener error:', listener);
    ...
    const TrackedIntersectionObserver: typeof window.IntersectionObserver = (function () {
      function WrappedObserver(
        ...
        if (typeof NativeIntersectionObserver === 'function') {
          const safeCallback = runSafely('Simulator legacy intersection observer error:', callback);
    ...
    try {
      ...
      failRuntime('Simulator legacy script execution error:', error);
    }
  ...
  return (
    <div ref={rootRef} id="dfp-heading-whatif" className="simulator-section-legacy" aria-label={'Simulador de Protocolo'}>
      <div ref={contentRef} dangerouslySetInnerHTML={{ __html: LEGACY_SIMULATOR_HTML }} />
    </div>
  );
};
